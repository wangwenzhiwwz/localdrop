...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => { try { const msg=JSON.parse(raw.toString()); if(msg.type==="register"){ clients.set(id,{ws,name:String(msg.name||"未知设备").slice(0,40),platform:String(msg.platform||"浏览器")}); broadcastPeers(); return; } const target=clients.get(msg.target); if(!target||target.ws.readyState!==WebSocket.OPEN)return; delete msg.target; target.ws.send(JSON.stringify({...msg,from:id,fromName:clients.get(id)?.name||"附近设备"})); } catch {} });
